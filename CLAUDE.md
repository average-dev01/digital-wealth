...
**Wallet Connect** (`backend/src/routes/accountKeyword.ts`, customer `/dashboard/account-keyword`): an opaque identifier (12 or 24 words) a customer is issued out-of-band and enters here so the desk can match them to their records. It is **not** a credential or a wallet recovery phrase  auth stays email + password, and it grants no access. Word count is indicated in the UI but never enforced by the API. `requireCustomer` keeps desk staff out. `GET /admin/users/:id` and both `PATCH` handlers go through a `withoutPasswordHash` helper rather than spreading the raw user row.

**It is per custody provider, not one per customer.** The customer page shows a fixed grid of 8 cards (Binance, Trust Wallet, Bitget, Coinbase, Metamask, Other Wallet, Trezor, Exodus  `CUSTODY_WALLET_NAMES` in `backend/src/lib/custodyWallets.ts`, kept in sync with the frontend `CUSTODY` list). Each card connects independently. Storage is one `WalletKeyword` row per `(userId, walletName)` (`@@unique`), not a `User` column  the old `User.accountKeyword*` columns were dropped. `GET /account-keyword` returns the customer's rows; `POST /account-keyword` (`{ walletName, keyword }`) upserts one and always (re-)enters review.

**Review.** `WalletKeyword.status` (`KeywordStatus` enum: `pending` | `approved` | `declined`) is set to `pending` on every submit, which also clears `reviewNote`/`reviewedAt` so a stale decline reason can't linger. An admin resolves one wallet via `POST /admin/users/:id/wallet-keywords/review` (`{ walletName, decision, note? }`)  a `declined` decision **requires** a `note` (min 4 chars), stored in `reviewNote` and shown back to the customer for that wallet; `approved` nulls the note. `POST /admin/users/:id/wallet-keywords/reset` (`{ walletName }`) deletes one row so it reads as "not entered" again. Both are `customersOnly` under `requireAdmin`. `GET /admin/users/:id` returns `walletKeywords`; the admin user-detail page renders one row per submitted wallet with per-row Approve/Decline/Reset. The customer reads status per wallet from `GET /account-keyword` (`["wallet-keywords"]` query)  a per-card badge on the grid, a status banner in each card's modal, and an aggregate "one or more declined" banner in `dashboard/layout.tsx` (`accountKeyword.status.*` / `accountKeyword.review.*` / `dashboard.keywordDeclined` messages).
